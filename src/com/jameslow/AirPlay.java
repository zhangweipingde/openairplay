...
public class AirPlay {
	...
	//AirPlay class
	public AirPlay(Service service) throws AWTException  {
		this(service.hostname,service.port,service.name);
	}
	public AirPlay(String hostname) throws AWTException  {
		this(hostname,PORT);
	}
	public AirPlay(String hostname, int port) throws AWTException  {
	...
	public static BufferedImage captureScreen() throws AWTException {
		return AirPlay.robot.createScreenCapture(AirPlay.rect);		 
	}
	...
	public class PhotoThread extends Thread {
		...
		public void run() {
			while (!Thread.interrupted()) {
				try {
					if (image == null) {
						airplay.photoRawCompress(airplay.scaleImage(AirPlay.captureScreen()), NONE);
					} else {
	...
	public static AirPlay searchDialog(Window parent) throws IOException, AWTException {
		return searchDialog(parent, 6000);
	}
	public static AirPlay searchDialog(Window parent, int timeout) throws IOException, AWTException {
		JDialog search = new JDialog(parent, "Searching...");
		search.setVisible(true);
		search.setBounds(0,0,400,200);
	...
	public static void selectResolutionDialog(Window parent, AirPlay airplay){
		...
		System.out.printf("Now airplaying at %dx%d\n", airplay.appletv_width, airplay.appletv_height);
